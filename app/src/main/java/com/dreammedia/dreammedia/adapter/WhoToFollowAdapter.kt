...
class WhoToFollowAdapter(val list: MutableList<WhoToFollowResponse.Responce>,
                         val listener: ClickListener, val contex: Context) : RecyclerView.Adapter<WhoToFollowAdapter.ViewHolder>() {

    interface ClickListener { fun onClickListener(pos: Int , id : Int , list :WhoToFollowResponse.Responce  ) }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WhoToFollowAdapter.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.list_follow , parent, false)
        return ViewHolder(v)
    }

    override fun onBindViewHolder(holder: WhoToFollowAdapter.ViewHolder, position: Int) {

        try {

            holder.tvName.setText(list.get(position).fullname)
            holder.tvDesignation.setText(list.get(position).email.toString())
